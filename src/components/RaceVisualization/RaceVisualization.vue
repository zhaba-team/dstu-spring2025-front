...
watch(members, (newMembers) => {
  const someoneFinished = newMembers.some(member => member.raceTime >= 100);

  if (!winnerDeclared.value && someoneFinished) {
    winnerDeclared.value = true;
    launchConfetti();
  }

  if (winnerDeclared.value && newMembers.every(m => m.raceTime < 100)) {
    // Забег начался заново — сбрасываем
    winnerDeclared.value = false;
  }
}, { deep: true });
...
<template>
  <div class="flex flex-col w-full text-left justify-between gap-5">
    ...
    <div class="raceBlock">
      <div class="flex gap-5 items-center" v-for="item in members">
        ...
        <ProgressBar :style="getStyle(item.color )" :value="item?.raceTime"></ProgressBar>
      </div>
    </div>
  </div>
</template>
